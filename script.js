...
/* 
  SECURITY WARNING — broker credentials:
  ---------------------------------------
  Any username/password in client-side JavaScript is visible to anyone who opens
  DevTools or "View Source". There is no way to keep them secret in a static web page.

  The workshop broker presets below include credentials intentionally so that
  workshop participants can connect without any extra setup. This is acceptable
  for a time-limited workshop with a semi-public broker, but is NOT appropriate
  for production use.

  For production:
  1. Use a public/anonymous broker topic for demos.
  2. Have the browser connect to your own backend, which then connects to MQTT
     (keeping real credentials server-side only).
  3. Issue short-lived tokens from a backend auth service and pass those to the
     broker instead of a long-lived password.
*/
